...
public class ItemsRepositoryForSync extends ItemsRepository {

	
	public ItemsRepositoryForSync(Context context) {
		super(context);
	}

	/*
	private static final String LOG_TAG = "ItemsRepositoryForSync";
	
    
    public Item getFirstNewlyCreated() {
    	Log.i(LOG_TAG, "Item getFirstNewlyCreated()");
        try { 
        	return itemsDao.queryBuilder().where().eq(Item.COLUMN_REMOTE_ID, 0).queryForFirst();
    ...
    public long createNewlyDownloaded(Item item) {
    	Log.i(LOG_TAG, "Item getFirstNewlyCreated()");
        try { 
	...
	public int updateSuccessPosted(Item item, long remote_id, Date synced_at) {
//		item.setSyncedAt(synced_at);
